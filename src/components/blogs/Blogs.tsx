...
const Blogs: React.FC = () => {
  ...
  const blogs = useMemo(() => {
    const list = dbBlogs
      .map((b: any) => {
        const author = b.authorId && typeof b.authorId === 'object' ? b.authorId : null;
        const authorId = author ? (author.id || author._id) : b.authorId;
        return {
          ...b,
          authorId,
          author: author || { name: 'Unknown', profileImage: '' },
          createdAt: b.createdAt || b.updatedAt || new Date().toISOString(),
          type: 'post',
        };
      })
      .filter((b: any) => b.type === 'post');
    const byQ = q ? list.filter((p: any) => (p.content + ' ' + (p.author?.name || '')).toLowerCase().includes(q.toLowerCase())) : list;
  ...
  return (
    <div className="space-y-6">
      ...
      {showComposer && (user?.role === 'alumni') && (
        <div className="p-4 border border-gray-200 rounded-xl bg-white/90">
            ...
            <div className="flex justify-end gap-3">
              <button onClick={()=>{ setShowComposer(false); setBlogText(''); setImageUrl(''); setImageData(''); }} className="px-4 py-2 border rounded-lg">Cancel</button>
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    const payload = {
                      ...
                      image: (imageData.trim() || imageUrl.trim() || autoImage),
                      category,
                      tags,
                      type: 'post',
                    };
                    const res = await apiFetch('/api/blogs', { method: 'POST', body: JSON.stringify(payload) });
                    if (!res.ok) {
                      const data = (await res.json().catch(() => ({}))) as any;
                      throw new Error(data.message || 'Failed to publish blog');
                    }
                    const created = await res.json();
                    setDbBlogs((prev) => [created, ...prev]);
                    setBlogText('');
                    setImageUrl('');
                    setImageData('');
                    setCategory('');
      ...
      {blogs.length === 0 && (
        <div className="bg-white/80 backdrop-blur rounded-2xl p-12 text-center shadow-elev-1 border border-white/50">
          <p className="text-gray-600">{loading ? 'Loading…' : 'No blogs match your filters.'}</p>
        </div>
      )}
    </div>
  );
